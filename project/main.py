...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost:3307/coviddbms'
db=SQLAlchemy(app)
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        if bedtype == "NormalBed" and dbb.normalbed > 0:
            ...
            flash("No available beds for selected type", "danger")
            return render_template("booking.html", query=query, query1=query1)

        db.session.commit()

        booking = Bookingpatient(
            srfid=srfid, bedtype=bedtype, hcode=hcode,
